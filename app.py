...
def generate_er_diagram():
    try:
        ...
        for file in csv_files:
            df = pd.read_csv(os.path.join(UPLOAD_FOLDER, file), sep='|', encoding='latin-1')
            dataframes[file] = df
...
def convert_expression():
    ...
    try:
        response = model.generate_content(f"Convert this tableau expression into dax expression. Give me the dax query as output not give the explanation {message}")
        print(response)
        
        if response.candidates:
            dax_expression = response.candidates[0].content.parts[0].text
            print(dax_expression)
            return jsonify({'dax_expression': to_markdown(dax_expression).data}), 200
        else:
